...
class apiGUI:
    def __init__(self, window):
        ...
        def state_selection(self,pwin):
            ...
            self.var_button = tk.Button(state_win, text = "Next",command = lambda:var_selection(self,state_win))
            self.var_button.grid(row=1,column=0, columnspan=2)
            #self.var_button.grid(row=0,column=0,columnspan=2)
        def var_selection(self, pwin):
            ...
            self.state_list_box = tk.Listbox(var_win, selectmode="multiple", listvariable=self.vtable)
            fill_listbox(self,self.state_list_box,self.vtable)
            self.state_list_box.grid(row=0,column=0,sticky='nsew')
            ...
            self.var_button = tk.Button(var_win, text = "Next") #,command = lambda:var_selection(x,cat_vars)
            self.var_button.grid(row=1,column=0, columnspan=2)
